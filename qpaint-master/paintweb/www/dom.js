...
class QLine {
    ...
    setProp(parent, key, val) {     //修改图形样式的某个属性
    ...
    onpaint(ctx) {              //View
        let theta = 30;
        let headlen = 10;
        let angle = Math.atan2(this.pt1.y-this.pt2.y,this.pt1.x-this.pt2.x)*180/Math.PI;
        ...
        arrowX = this.pt2.x+topX;
        arrowY = this.pt2.y+topY;
        ctx.moveTo(arrowX,arrowY);
        ctx.lineTo(this.pt2.x, this.pt2.y);
        arrowX = this.pt2.x+botX;
...
class QPaintDoc {           //Model
    ...
    deleteShape(shape) {        //Controllers，删除某个图形
        deleteItem(this._shapes, shape)
        documentChanged(this)
    }
    hitTest(pt) {       //Controllers，确定鼠标点中哪个图形
        let shapes = this._shapes
        let n = shapes.length
        for (let i = n-1; i >= 0; i--) {
